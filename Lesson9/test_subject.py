...
def test_add_subject(db):
    ...
    insert_stmt = text("INSERT INTO subject (subject_id, subject_title) VALUES (:id, :title)")
    result = db.execute(insert_stmt, {"id": new_id, "title": "Test"})
...
def test_edit_subject(db):
    ...
    db.execute(text("INSERT INTO subject (subject_id, subject_title) VALUES (:id, :title)"),
               {"id": new_id, "title": "History2"})

    db.execute(text("UPDATE subject SET subject_title = :new_title WHERE subject_id = :id"),
               {"new_title": "World History", "id": new_id})

    subject = db.execute(text("SELECT * FROM subject WHERE subject_id = :id"), {"id": new_id}).fetchone()
